...
def add_habit(db, habit_name, habit_periodicity, habit_duration, streak, streak_type, date_last_completed, current_time):
    ...
    """
    cursor = db.cursor()
    cursor.execute("INSERT INTO habits VALUES (?, ?, ?, ?, ?, ?, ?)", (habit_name,
                   habit_periodicity, habit_duration, streak, streak_type, date_last_completed, current_time))
    db.commit()
...
def first_time_check_off(db, habit_name, current_completed_time, streak_type):
    ...
    cursor.execute(
        f"UPDATE habits SET date_last_completed = '{current_completed_time}' WHERE habit_name = '{habit_name}';")
    cursor.execute(
        f"UPDATE habits SET streak = 1 WHERE habit_name = '{habit_name}';")
    db.commit()
    streak = cursor.execute(
        f"SELECT streak FROM habits WHERE habit_name = '{habit_name}';").fetchone()[0]
    logger = Log(habit_name, first_completed_time,
                 current_completed_time, streak, streak_type)
    logger.add()
...
def check_off(db, habit_name, current_completed_time, last_completed, streak_type):
    ...
    """
    cursor = db.cursor()
    first_completed_time = cursor.execute(
        f"SELECT first_completed_time FROM log WHERE last_completed_date = '{last_completed}'").fetchone()[0]
    cursor.execute(
        f"UPDATE habits SET date_last_completed = '{current_completed_time}' WHERE habit_name = '{habit_name}';")
    cursor.execute(
        f"UPDATE habits SET streak = streak + 1 WHERE habit_name = '{habit_name}';")
    db.commit()
    streak = cursor.execute(
        f"SELECT streak FROM habits WHERE habit_name = '{habit_name}';").fetchone()[0]
    logger = Log(habit_name, first_completed_time,
...
def reset_check_off(db, habit_name, current_completed_time, streak_type):
    ...
    cursor.execute(
        f"UPDATE habits SET streak = 1 WHERE habit_name = '{habit_name}';")
    db.commit()
    logger = Log(habit_name, current_completed_time,
                 current_completed_time, '1', streak_type)
    logger.add()
    db.commit()
...
def log_inactivity(habit_name, last_inactive_time, first_inactive_time, inactive_streak, inactive_streak_type):
    ...
    """
    inactive_logger = InactiveLog(habit_name, last_inactive_time,
                                  first_inactive_time, inactive_streak, inactive_streak_type)
    inactive_logger.add()
...
def get_dates(last_completed, current_completed_time):
    ...
    d2 = datetime.datetime.strptime(
        current_completed_time, "%d/%m/%Y %H:%M").date()
    return d1, d2
...
def mark_habit_done(db, habit_name):
    ...
    """
    cursor = db.cursor()
    habit_periodicity = cursor.execute(
        f"SELECT periodicity FROM habits WHERE habit_name = '{habit_name}';").fetchone()[0]
    last_completed = cursor.execute(
        f"SELECT date_last_completed FROM habits WHERE habit_name = '{habit_name}';").fetchone()[0]
    current_completed_time = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")

    if (habit_periodicity == "Daily"):
        streak_type = "day(s)"
        if (last_completed == None):
            first_time_check_off(
                db, habit_name, current_completed_time, streak_type)
            print(
            ...
            if (diff == 0):
                ...
                check_off(db, habit_name, current_completed_time,
                          last_completed, streak_type)
                streak = cursor.execute(
                    f"SELECT streak FROM habits WHERE habit_name = '{habit_name}';").fetchone()[0]
                ...
                log_inactivity(habit_name, last_completed,
                               current_completed_time, diff, streak_type)
                reset_check_off(
                    db, habit_name, current_completed_time, streak_type)
...
def update_habit(old_name, db, new_name, new_periodicity, new_duration):
    """
    ...
    cursor = db.cursor()
    cursor.execute(
        f"UPDATE habits SET habit_name = '{new_name}' WHERE habit_name = '{old_name}';")
    cursor.execute(
        f"UPDATE habits SET periodicity = '{new_periodicity}' WHERE habit_name = '{new_name}';")
    cursor.execute(
        f"UPDATE habits SET duration = '{new_duration}' WHERE habit_name = '{new_name}';")

    # Also update the log and inactive_log tables
    cursor.execute(
        f"UPDATE log SET habit_name = '{new_name}' WHERE habit_name = '{old_name}';")
    cursor.execute(
        f"UPDATE inactive_log SET habit_name = '{new_name}' WHERE habit_name = '{old_name}';")
